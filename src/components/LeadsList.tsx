...
import { Lead, LeadFilters, SortConfig } from '../types/crm';
...
const LeadsList: React.FC<LeadsListProps> = ({ leads, onLeadSelect, onConvertToOpportunity, onLeadDelete }) => {
  ...
  const handleDelete = async (leadId: string) => {
    ...
    try {
      ...
      if (Math.random() < 0.10) {
        throw new Error('Erro ao excluir lead. Verifique sua conexão e tente novamente.');
      }
      ...
      setError(err instanceof Error ? err.message : 'Erro desconhecido ao excluir lead');
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <div className="bg-white/60 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200/50">
      ...
      <div className="p-6 sm:p-8 border-b border-gray-200/50">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <h2 className="text-2xl font-bold text-gray-900">Leads ({filteredAndSortedLeads.length})</h2>
        ...
        {successMessage && (
          ...
          <div className="flex-1 max-w-md relative">
            ...
            <input
              ...
              disabled={isLoading}
              className={`w-full pl-12 pr-4 py-4 bg-white/80 border border-gray-200/50 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 transition-all duration-200 placeholder-gray-400 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
            />
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row flex-wrap gap-3">
          ...
          <div className="relative">
            <select
              ...
              disabled={isLoading}
              className={`w-full sm:w-auto appearance-none bg-white/80 border border-gray-200/50 rounded-xl px-4 py-3 pr-10 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 transition-all duration-200 cursor-pointer ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
          ...
          {(filters.status || filters.source || searchTerm) && (
            <button
              ...
              disabled={isLoading}
              className={`w-full sm:w-auto px-6 py-3 text-sm font-medium text-gray-600 hover:text-gray-900 bg-white/80 border border-gray-200/50 rounded-xl hover:bg-white transition-all duration-200 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              Limpar Filtros
            </button>
          )}
        ...
        {filteredAndSortedLeads.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-light text-gray-900 mb-3">
              {searchTerm || filters.status || filters.source ? 'Nenhum lead encontrado' : 'Nenhum lead disponível'}
            </h3>
                  ...
                  <tr key={lead.id} className="hover:bg-gray-50/50 transition-colors duration-200">
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="flex items-center">
                        <div>
                          ...
                          <div className="text-sm text-gray-500 font-light">{lead.email}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-gray-500 font-light">{lead.position}</div>
                      </div>
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap">
                      <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${getStatusColor(lead.status)}`}>
                        {lead.status}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-5 whitespace-nowrap text-sm font-medium space-x-3">
                      ...
                      <button
                        ...
                        className={`text-red-600 hover:text-red-800 transition-colors duration-200 font-medium ${deletingId === lead.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                      >
      ...
      <div className="md:hidden space-y-3 relative">
        ...
        {filteredAndSortedLeads.length === 0 ? (
          ...
          filteredAndSortedLeads.map((lead) => (
            <div key={lead.id} className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/50 p-5 hover:shadow-lg hover:shadow-gray-200/50 transition-all duration-300">
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-medium text-gray-900 truncate">{lead.name}</h3>
                  <p className="text-sm text-gray-500 font-light truncate">{lead.email}</p>
                  <p className="text-sm text-gray-600 font-medium mt-1">{lead.company}</p>
                </div>
                <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ml-3 ${getStatusColor(lead.status)}`}>
                  {lead.status}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                ...
                <div className="space-y-1">
                  <div className="text-xs text-gray-500 font-medium uppercase tracking-wide">Fonte</div>
              ...
              <div className="flex items-center justify-between text-xs text-gray-500 mb-4 pb-4 border-b border-gray-200/50">
                <span className="font-light">Criado em {new Date(lead.createdAt).toLocaleDateString('pt-BR')}</span>
              </div>
              ...
              <div className="flex space-x-2">
                ...
                <button
                  ...
                  disabled={deletingId === lead.id}
                  className={`px-4 py-2.5 text-sm text-red-600 bg-red-50/80 rounded-xl hover:bg-red-100/80 font-medium transition-all duration-200 ${deletingId === lead.id ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {deletingId === lead.id ? 'Excluindo...' : 'Excluir'}
                </button>
              </div>
